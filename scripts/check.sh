...
if step build bash scripts/cargo.sh test --offline --locked --no-run; then
  ...
  lane alone check-ip-location check-bind check-link-plugin check-launcher check-theme check-map-tiles check-map-overlay test-engine-pin check-engine-release check-popover check-reconnect & lanes+=($!)
